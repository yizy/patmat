...
object Huffman {
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    freqs.foldLeft(List[Leaf]()) ((l, e) => new Leaf(e._1, e._2) :: l).sortWith(_.weight < _.weight)
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def impl(t: CodeTree, b: List[Bit], chars: List[Char]): List[Char] = (t, b) match {
      case (leaf: Leaf, x) => impl(tree, x, leaf.char :: chars)
      case (fork: Fork, a :: tail) =>
        if (a == 0) impl(fork.left, tail, chars) else impl(fork.right, tail, chars)
      case (_, _) => chars
    }

    impl(tree, bits, List[Char]()).reverse
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def getBits(t: CodeTree, c: Char): List[Bit] = t match {
      case Fork(l, _, _, _) if chars(l).contains(c) => 0 :: getBits(l, c)
      case Fork(_, r, _, _) if chars(r).contains(c) => 1 :: getBits(r, c)
      case _ => Nil
    }

    text.flatMap(c => getBits(tree, c))
  }
...
object Test extends App {
  println(Huffman.times("abcdabcaba".toList))
